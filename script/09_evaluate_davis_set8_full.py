...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Full DAVIS+Set8 evaluation for Baseline vs OOTW-3DNR.")
    p.add_argument(
        "--base_root",
        type=str,
    ...
    p.add_argument("--out_json", type=str, default="metrics_davis_set8_full.json")
    p.add_argument(
...
def discover_set8_sequences(set8_root: Path, max_frames: int) -> Dict[str, List[Path]]:
    ...
    # Consider all leaf-like directories that contain image frames directly.
...
def evaluate_sequence(
    ...
    psnr_ours: List[float] = []
    ssim_ours: List[float] = []
    ...
    for i, fp in enumerate(frames, start=1):
        ...
        # Baseline with previous processed frame as reference.
        ...
        # Ours with current-noisy feature extraction + ordinal w_map inference.
        ...
        pb, sb = compute_psnr_ssim(clean, out_baseline)
        po, so = compute_psnr_ssim(clean, out_ours)
        psnr_base.append(pb)
        ssim_base.append(sb)
        psnr_ours.append(po)
        ssim_ours.append(so)
    ...
    return {
        "traditional_baseline": {
            "psnr": summarize(psnr_base),
            "ssim": summarize(ssim_base),
            "num_frames": int(n),
            "psnr_values": psnr_base,
            "ssim_values": ssim_base,
        },
        "ours_ootw_3dnr": {
            "psnr": summarize(psnr_ours),
            "ssim": summarize(ssim_ours),
            "num_frames": int(n),
            "psnr_values": psnr_ours,
            "ssim_values": ssim_ours,
        },
    }
...
def main() -> None:
    ...
    tdnr_mod = load_module(tdnr_py, "tdnr_module_eval09")
    oracle_mod = load_module(oracle_lib_py, "oracle_module_eval09")
    ...
    for dsi, (dataset_name, seq_map) in enumerate(datasets.items()):
        for sigma in sigmas:
            ...
            dataset_method_psnr: Dict[str, List[float]] = {m: [] for m in methods}
            dataset_method_ssim: Dict[str, List[float]] = {m: [] for m in methods}
            ...
            for seqi, (seq_name, frames) in enumerate(seq_map.items()):
                ...
                # Keep per-clip summary + raw values
                clip_store: Dict[str, Any] = {}
                for m in methods:
                    clip_store[m] = {
                        "psnr": seq_result[m]["psnr"],
                        "ssim": seq_result[m]["ssim"],
                        "num_frames": int(seq_result[m]["num_frames"]),
                    }
                    dataset_method_psnr[m].extend(seq_result[m]["psnr_values"])
                    dataset_method_ssim[m].extend(seq_result[m]["ssim_values"])
                results[dataset_name][sigma_key]["clips"][seq_name] = clip_store

            for m in methods:
                results[dataset_name][sigma_key]["dataset_mean"][m] = {
                    "psnr": summarize(dataset_method_psnr[m]),
                    "ssim": summarize(dataset_method_ssim[m]),
                    "num_frames": int(len(dataset_method_psnr[m])),
                }
    ...
    out_payload = {
        ...
        "results": results,
        "markdown_summary_table_psnr_ssim": markdown_table,
    }
    ...
    print("\n=== Markdown Summary Table (PSNR/SSIM) ===")
